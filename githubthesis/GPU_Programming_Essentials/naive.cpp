...
int main(){
   ...
   for (int iter =1;iter<=5;iter++){
    for(int u=0;u<5;u++){
        smat[0][0] = 0; smat[0][1] = 0;
        smat[1][0] = 0; smat[1][1] = 0;
        svec[0] = 0; svec[1] = 0;
        for(int i=0;i<5;i++){
            if(R[u][i]==0) continue;
   ...
   for(int i=0;i<5;i++){
    for(int j=0;j<5;j++){
        ...
        // Clipping: রেটিং ০ এর নিচে নামলে ০ এবং ৫ এর উপরে গেলে ৫ করে দেয়া হচ্ছে
        if(r_predic[i][j] < 0) r_predic[i][j] = 0;
        if(r_predic[i][j] > 5) r_predic[i][j] = 5;
